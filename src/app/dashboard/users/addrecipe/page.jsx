...
const AddRecipe = () => {
  ...
  // Authentication status using Client Auth Hook
  const { data: session, isPending } = authClient.useSession();
  const user = session?.user;

  console.log(user.id ,"the one ");
  console.log(user,"the one ttttt ");
  ...
  const handleRecipeSubmit = async (e) => {
    ...
    try {
      ...
      // 2. Upload image via Server Action
      const imgFormData = new FormData();
      imgFormData.append("image", imageFile);

      const image = await imageupload(imgFormData);

      if (!image || !image.url) {
        throw new Error("Image upload failed. Please check your API key or connection.");
      }
      ...
      console.log(result.userId, 'recipe result id');
    ...
    >
      ...
      <form onSubmit={handleRecipeSubmit} className="space-y-4">
        ...
        <button
          ...
          className="w-full bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white font-bold py-3 rounded-xl transition text-xs font-mono uppercase tracking-widest flex items-center justify-center gap-2 cursor-pointer"
        >
          {submitting ? (
            <>
...
//     // 1. ফাইল পাস করার জন্য নতুন FormData তৈরি
//     const imgFormData = new FormData();
//     imgFormData.append("image", imageFile);
...
//     // 3. Safe Check (image বা image.url না থাকলে কোড এখানেই থেমে যাবে)
//     if (!image || !image.url) {
//       throw new Error("Image upload failed. Please check your API key or connection.");
//     }
...
//     // 4. Save Recipe in Database
//     const resullt = await addrecipe({ 
//       ...data, 
//       image: image.url, 
//       userId: user.id 
//     });
