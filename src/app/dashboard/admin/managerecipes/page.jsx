...
import { FaTrash, FaEdit, FaStar, FaRegStar, FaSpinner } from 'react-icons/fa';
import { getAllRecipes, toggleFeaturedRecipe, deleteRecipe } from '@/lib/actions/recipeActions';
...
const ManageRecipes = () => {
  ...
  // Featured স্ট্যাটাস টগল
  const handleToggleFeatured = async (recipeId, currentFeatured) => {
    setActionLoadingId(recipeId);
    const res = await toggleFeaturedRecipe(recipeId, currentFeatured);
    if (res.success) {
      setRecipes((prev) =>
        prev.map((r) => (r._id === recipeId ? { ...r, isFeatured: !currentFeatured } : r))
      );
    } else {
            ...
            {recipes.map((recipe) => (
              <tr key={recipe._id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition">
                ...
                <td className="py-4 px-6 text-gray-600 dark:text-gray-400">
                  {recipe.author || recipe.userEmail || 'Unknown'}
                </td>
                <td className="py-4 px-6 text-gray-600 dark:text-gray-400">
                ...
                <td className="py-4 px-6">
                  <button
                    ...
                    disabled={actionLoadingId === recipe._id}
                ...
                <td className="py-4 px-6 text-right space-x-2">
                  <button
                    onClick={() => alert(`Edit Modal for ${recipe.title || recipe.name}`)}
                    className="p-2 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 dark:bg-blue-950 dark:text-blue-400 cursor-pointer"
                  >
                    <FaEdit />
                  </button>
                  <button
                    ...
                    disabled={actionLoadingId === recipe._id}
                    className="p-2 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 dark:bg-red-950 dark:text-red-400 disabled:opacity-50 cursor-pointer"
                  >
                    <FaTrash />
                  </button>
                </td>
