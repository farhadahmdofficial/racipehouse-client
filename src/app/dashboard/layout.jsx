...
const userLinks = [
  ...
  { name: 'Profile', href: '/dashboard/profile', icon: FaUser },
  { name: 'Your plan', href: '/pricing', icon: IoPricetagsOutline },
];
...
const DashboardLayout = ({ children }) => {
  ...
  // 1. Session এবং Pending State ফেচিং
  const { data: session, isPending } = authClient.useSession();
  const user = session?.user;

  // 2. সেশন না থাকলে অটোমেটিক লগইন পেজে রিডাইরেক্ট
  useEffect(() => {
  ...
  // 3. লোডিং অবস্থায় স্ক্রীন ব্ল্যাঙ্ক না রেখে স্পিনার দেখানো
  if (isPending) {
    return (
  ...
  // সেশন না পাওয়া গেলে কিছু রেন্ডার করবে না (রিডাইরেক্ট হওয়া পর্যন্ত)
  if (!session) {
    return null;
  }
